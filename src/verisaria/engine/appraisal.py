...
class RelationshipAppraiser:
    ...
    def _build_prompt(
        ...
        return (
            ...
            "判断要双向、对称：不要一遇到对方坚持或施压就只会加深怀疑。若对方此刻**坦诚、"
            "讲道理、展现善意或确实为你着想**，你的戒备(suspicion)应当**下降**、信任(trust)或"
            "敬重(respect)可以**上升**——给出**负**的 suspicion 增量是完全正常、也是被鼓励的。"
            "只有当对方**真的**显出威胁、欺骗或敌意时才加深怀疑。一句无关痛痒的话则几乎不该改变什么。\n"
